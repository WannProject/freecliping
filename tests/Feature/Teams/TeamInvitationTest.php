...
test('invitation email for existing users uses home route', function () {
    $owner = User::factory()->create();
    ...
    expect($mail->actionUrl)->toBe(route('home', ['invitation' => $invitation->code]));
    $this->assertStringContainsString('administrator', implode(' ', $mail->introLines));
});

test('invitation email for unknown users uses home route', function () {
    $owner = User::factory()->create();
    $team = Team::factory()->create();
